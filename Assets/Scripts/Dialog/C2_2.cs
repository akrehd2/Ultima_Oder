        ...
        if (DialogSystem.instance.day == 1)
        {
            ...
            else if (DialogSystem.instance.Result == 1)
            {
                if (DialogSystem.instance.Rang == 0)
                {
                    ...
                    dialogTexts.Add(new DialogData("\"At first, I hope it's romantic,\" what's the closest ingredient we have in this spell?", "제니퍼", () => Show_RoseE()));
                ...
                else
                {
                    ...
                    dialogTexts.Add(new DialogData("\"처음에는 로맨틱함이 사로잡았으면 좋겠고요,\" 이 주문에서 우리가 갖고있는 향들 중에 제일 근접한 재료가 뭘까?", "제니퍼", () => Show_RoseK()));
